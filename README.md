...
LINE トーク内で以下のような自動応答が動作します（スクリーンショットは別途追加予定）。

- リッチメニューから「車検予約」をタップ → 第 1〜第 3 希望日時の選択 → 名前・電話入力 → 予約完了
- リッチメニューから「修理相談」をタップ → 症状入力 → 写真アップロード → 希望時期入力 → 依頼送信
- 「予約確認」で既存予約の一覧表示・変更・キャンセル
